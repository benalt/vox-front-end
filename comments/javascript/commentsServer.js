...
function renderBasePage(callback, startIdx) {
  ...
  jsdom.env({
    html: '<html><head></head><body><div id="vox-doc"><h2 class="vox-subtitle">Comments</h2><div id="vox-cmnt-container"></div></div></body></html>',
    scripts: [
    ...
    $.each(getFromIdx(0,initialLoadNumber), function(idx,ctx){
...
function addRandomComment() {
  var to = setTimeout(function(){ clearTimeout(to); addRandomComment(); }, 20000)
  ...
  if (randomTypeFlag < 2) {
    ...
    if (randomTypeFlag < 1) {
       ...
       newComment.replyTo = commentedUpon.id;
    }
    randomTypeFlag += 1;
  } else {
    commentedUpon.recommended = true;
    randomTypeFlag = 0;
  }  
}
